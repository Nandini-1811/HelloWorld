...
static void print_operand(IROperand o)
{
    ...
    {
    case OPERAND_TEMP:
        printf("t%d", o.val.temp_id);
        break;
    case OPERAND_VAR:
        printf("%s", o.val.name);
        break;
    case OPERAND_INT:
        printf("%d", o.val.int_val);
        break;
    case OPERAND_FLOAT:
        printf("%g", o.val.float_val);
        break;
    case OPERAND_STRING:
        printf("\"%s\"", o.val.str_val);
        break;
    case OPERAND_LABEL:
        printf("%s", o.val.name);
        break;
...
void ir_print(IRProgram *p)
{
    ...
    for (int i = 0; i < p->count; i++)
    {
        ...
        case IR_ADD:
            print_operand(in->dest);
            printf(" = ");
            print_operand(in->src1);
            printf(" + ");
            print_operand(in->src2);
            break;
        case IR_SUB:
            print_operand(in->dest);
            printf(" = ");
...
static void gen_stmt(IRProgram *p, ASTNode *node)
{
    ...
    case NODE_FUNC_DECL:
    {
        ...
        ir_emit(p, (IRInstruction){
                       IR_LABEL,
                       op_label(strdup(node->as.func_decl.name)),
                       op_none(), op_none()});

        /* ── load parameters from call stack into variables ── */
        /* emit PARAM_LOAD for each parameter in order          */
        for (int i = 0; i < node->as.func_decl.param_count; i++)
        {
            ir_emit(p, (IRInstruction){
                           IR_LOAD,
                           op_var(node->as.func_decl.params[i].name),
                           op_int(i), /* index into call stack */
                           op_none()});
        }
...
IRProgram *generate_ir(ASTNode *root)
{
    ...
    {
        if (root->as.program.statements[i]->type == NODE_FUNC_DECL)
            gen_stmt(p, root->as.program.statements[i]);
    }

    /* second pass — emit global code under a main label */
    ir_emit(p, (IRInstruction){
                   IR_LABEL,
                   op_label(strdup("__main")),
                   op_none(), op_none()});
